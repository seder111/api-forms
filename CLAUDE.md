...
**Split deployment model** — the private app (`bootstrap.php`, `src/`, `vendor/`, `.env`, `storage/`) lives *outside* the public webroot; only `doc_public/api/` (entry point `index.php` + `.htaccess`) gets copied into `public_html/api/`. `doc_public/api/index.php` locates `bootstrap.php` two directories up (falls back to same-directory for local dev testing). This means routing/business logic changes belong in `src/`, but a change to the public entry contract belongs in `doc_public/api/`.

**Request flow**: `doc_public/api/.htaccess` rewrites all non-file/non-dir requests to `index.php` → `bootstrap.php` loads `.env` via `Dotenv::createImmutable()->safeLoad()`, sets error display based on `APP_DEBUG`, requires `src/routes.php`, then calls `Flight::start()`. Routes are defined with Flight's static `Flight::route()` API and dispatch to controller static methods (see `src/routes.php`, `src/Http/ContactController.php`).
...
**Field contract is a two-sided agreement**: HTML `name` attributes in the consuming site's form must exactly match what the controller reads from `$_POST` (currently `nom`, `email`, `privacitat`, `newsletter`). When adapting this backend for a new form/site, both the frontend markup and the PHP controller must change together — HTML `required`/`type` attributes are UX only, real validation always happens again in PHP.
